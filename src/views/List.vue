    ...
    <div class="text-left">
        ...
        <!-- <ul class="text-center px-10">
            ...
            <li v-for="(item, index) in userList" :key="item.id"
                class="list-none flex justify-start items-center h-16 border-b border-solid border-gray-700">
        ...
        <span class="flex grow justify-around">
          <a  @click="deleteUser(index)">删除</a>
          
          <a @click="editUser(index)">编辑</a>
          
          <a @click="getYourName(record.id)">问名字</a>
        </span>
      </template>
    </template>
        </a-table> 
        ...
        <a-modal v-model:open="showFlag" :width="600" @cancel="cancelFn" @ok="submitFn" okText="确定" cancelText="取消"
            :title="isEdit ? '编辑同学' : '新增同学'">
            <a-input v-model:value="studyNum" placeholder="请输入学号" style="width: 300px;margin: 10px;"/>
            <a-input v-model:value="name" placeholder="请输入姓名" style="width: 300px;margin: 10px;">
                <template #prefix>
                    <user-outlined />
                </template>
                <template #suffix>
            ...
            <a-input-number  v-model:value="year" :min="1" :max="25" placeholder="请输入年龄" style="width: 300px;margin: 10px;">
                <template #upIcon>
                    <ArrowUpOutlined />
                </template>
                <template #downIcon>
